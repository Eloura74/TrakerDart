...
async function generateShareImage(volley: Volley): Promise<Blob> {
  ...
  ctx.fillText(`${volley.comparison.consistencyIndex}%`, 600, 300);
...
async function shareOnPlatform(options: SocialShareOptions): Promise<void> {
...
- [x] Export PDF avec graphiques ✅ **IMPLÉMENTÉ**
